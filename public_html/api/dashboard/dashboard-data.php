...
// ======================================================
// 3) TOTVS 000070 + 000071: CACHE 2 MIN + FORCE
// ======================================================
$cacheMinutes = 2;
...
if (!$useCache) {
  ...
  if (!empty($resp70['success']) && is_array($resp70['data'])) {
    ...
    foreach ($items70 as $row) {
      ...
      if ($inRange($ts, $fromYear, $toToday))
        $fatYear += $valor;
      if ($inRange($ts, $fromMonth, $toToday))
        $fatMonth += $valor;
      if ($emissao === $todayStr)
        $fatToday += $valor;
    }
  ...
  if (!empty($resp71['success']) && is_array($resp71['data'])) {
    ...
    foreach ($items71 as $row) {
      ...
      if ($tsEmissao === null && $tsFecent === null)
        continue;

      $valor = (float) ($row['VALOR_PEDIDO'] ?? $row['VALOR'] ?? $row['VALOR_TOTAL'] ?? 0);

      // Só considera C5_FECENT.
      // Se não estiver preenchida ou for inválida, não contabiliza.
      if ($tsFecent === null) {
        continue;
      }

      if ($isIM) {
        if ($inRange($tsFecent, $fromMonth, $toMonthEnd))
          $imMonth += $valor;
        if ($fecent === $todayStr)
          $imToday += $valor;
        if ($inRange($tsFecent, $fromYear, $toYearEnd))
          $imYear += $valor;
      }
