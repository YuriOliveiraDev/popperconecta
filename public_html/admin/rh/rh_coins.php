...
require_once APP_ROOT . '/app/services/poppers_coins.php';
...
if (isset($_GET['success'])) {
  if ($_GET['success'] === 'launch') {
    $qty = (int) ($_GET['qty'] ?? 0);
    $success = 'Lançamento registrado com sucesso para ' . $qty . ' usuário(s).';
  }
}
...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
  ...
  try {
    ...
    $qty = count($userIds);
    header('Location: ' . $_SERVER['PHP_SELF'] . '?success=launch&qty=' . $qty);
    exit;
  } catch (Throwable $e) {
    if ($db->inTransaction()) {
      $db->rollBack();
...
<head>
  ...
  <link rel="stylesheet" href="/assets/css/rh_coins.css?v=<?= filemtime(APP_ROOT . '/assets/css/rh_coins.css') ?>" />
</head>
...
<section class="coins-card coins-card--mt">
  <div class="coins-card__head">
    <h3 class="coins-card__title">Histórico de lançamentos</h3>
    <span class="coins-card__badge">Todos os registros</span>
  </div>
  ...
  <div class="ledger-scroll">
    <table class="table" id="ledgerTable">
      ...
      <tbody>
        ...
        <?php if (!$ledger): ?>
          <tr>
            <td colspan="6" class="muted">Nenhum lançamento.</td>
          </tr>
        <?php endif; ?>
      </tbody>
    </table>
  ...
  <script src="/assets/js/rh_coins.js?v=<?= filemtime(APP_ROOT . '/assets/js/rh_coins.js') ?>" defer></script>
