    ...
    if ($action === 'deactivate' && $id > 0) {
      ...
      if (!empty($row['image_url']) && is_string($row['image_url'])) {
        ...
        if (str_starts_with($row['image_url'], $prefix)) {
          $filePath = APP_ROOT . $row['image_url'];
          if (is_file($filePath)) {
            @unlink($filePath);
          }
        }
      ...
      if (isset($_FILES['image']) && $_FILES['image']['error'] === UPLOAD_ERR_OK) {
        ...
        $uploadDir = APP_ROOT . '/uploads/rewards/';
        if (!is_dir($uploadDir)) {
...
<head>
  ...
  <link rel="stylesheet"
    href="/assets/css/rh_rewards.css?v=<?= filemtime(APP_ROOT . '/assets/css/rh_rewards.css') ?>" />
...
<body class="page">
  ...
  <script src="/assets/js/rh_rewards.js?v=<?= filemtime(APP_ROOT . '/assets/js/rh_rewards.js') ?>" defer></script>
  <?php
  $fiPath = APP_ROOT . '/assets/js/file-inputs.js';
  $fiVer = is_file($fiPath) ? (string) filemtime($fiPath) : (string) time();
  ?>
  <script src="/assets/js/file-inputs.js?v=<?= htmlspecialchars($fiVer, ENT_QUOTES, 'UTF-8') ?>" defer></script>
</body>
