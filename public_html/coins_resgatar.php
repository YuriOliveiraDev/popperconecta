...
if ($_SERVER['REQUEST_METHOD'] === 'POST' && isset($_POST['redeem_reward_id'])) {
  try {
    ...
    // impede pedido pendente duplicado da mesma recompensa
    $stmt = $db->prepare("
      SELECT COUNT(*)
    ...
    $observacaoSolicitante = $userNote;
...
<head>
  ...
  <title>Loja PopperCoins — <?= htmlspecialchars((string) APP_NAME, ENT_QUOTES, 'UTF-8') ?></title>
...
<body class="page">
  ...
  <main class="container coins">
    <div class="pc-header">
      <h1 class="pc-title">Loja Popper Coins</h1>
