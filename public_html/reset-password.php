...
function findValidResetByToken(PDO $pdo, string $token): ?array
{
    $stmt = $pdo->query("
...
if ($token === '') {
    ...
    $tokenIsValid = findValidResetByToken($pdo, $token) !== null;
    if (!$tokenIsValid) {
        $error = 'Este link é inválido ou expirou.';
    }
}

if (($_SERVER['REQUEST_METHOD'] ?? '') === 'POST' && $token !== '' && $tokenIsValid) {
    ...
    if ($password === '' || $confirm === '') {
        ...
        try {
            $found = findValidResetByToken($pdo, $token);
        ...
        <section class="auth-panel">
            ...
            <section class="auth-right">
                <div class="auth-card">
                    ...
                    <?php else: ?>
                        <a class="auth-btn" href="/login.php">
                            <span class="auth-btn__text">Voltar ao login</span>
                        </a>
                    <?php endif; ?>
