...
function upload_profile_photo_for_user(int $userId): array
{
  ...
  $dir = APP_ROOT . '/admin/uploads/profile_photos';
  if (!is_dir($dir)) {
    @mkdir($dir, 0775, true);
  }
  ...
  return ['ok' => true, 'path' => '/admin/uploads/profile_photos/' . $fileName, 'error' => null];
}
...
if ($profilePhotoPath !== '' && $profilePhotoPath[0] !== '/') {
  $profilePhotoPath = '/admin/uploads/profile_photos/' . ltrim($profilePhotoPath, '/');
}
  ...
  if ((int) $me['id'] === (int) $id) {
    ...
    try {
      ...
      if ($stmt->rowCount() < 1) {
        ...
        header('Location: /admin/users/users.php?deleted=1');
        exit;
      }
    } catch (Throwable $e) {
...
<head>
  ...
  <link rel="stylesheet" href="/assets/css/user_edit.css?v=<?= filemtime(APP_ROOT . '/assets/css/user_edit.css') ?>" />
  ...
  <main class="container user-edit">
    <section class="user-edit-hero">
      <div class="user-edit-hero__content">
        <div class="user-edit-hero__actions">
          <a class="btn-ghost" href="/admin/users/users.php"></a>
        </div>
      </div>
    </section>
    ...
    <section class="user-edit-shell">
      <div class="user-edit-card">
        ...
        <form method="post" class="form form--edit" action="/admin/users/user_edit.php?id=<?= (int) $user['id'] ?>"
