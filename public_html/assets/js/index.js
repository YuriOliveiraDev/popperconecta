  ...
  function loadCharts(v) {
    ...
    // RITMO
    const diasTrab = Math.max(1, num(v.dias_uteis_trabalhados));
    const totalMesPrincipal = num(v.mes_total) || num(v.realizado_ate_hoje);

    const metaDia = num(v.meta_dia_util);
    const hojePrincipal = num(v.hoje_total) || (num(v.hoje_faturado) + num(v.hoje_im));

    if (chartRitmo) chartRitmo.destroy();
    chartRitmo = new Chart(document.getElementById("chartRitmo"), {
      type: "bar",
      data: {
        labels: ["Meta do dia (teórica)", "Realizado hoje (Fat + IM)"],
        datasets: [{
          borderSkipped: "bottom",
          borderRadius: { topLeft: 14, topRight: 14, bottomLeft: 0, bottomRight: 0 },
