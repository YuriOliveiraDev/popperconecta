...
$getUF = function (array $row): string {
  $possible = [
    'UF',
    'ESTADO',
    'EST',
...
$ufToRegiao = function (string $uf): string {
  static $map = [
  'AC' => 'Norte',
  'AP' => 'Norte',
  'AM' => 'Norte',
...
foreach ($items as $row) {
  ...
  if ($ts < (int) $clients[$key]['first_ts'])
    $clients[$key]['first_ts'] = $ts;
  if ($ts > (int) $clients[$key]['last_ts'])
    $clients[$key]['last_ts'] = $ts;
  ...
  $ymd = date('Y-m-d', $ts);
  if (!isset($daysByClient[$key]))
    $daysByClient[$key] = [];
  if (!isset($daysByClient[$key][$ymd]))
    $daysByClient[$key][$ymd] = 0.0;
  $daysByClient[$key][$ymd] += $valor;

  // NOVO: agrega por UF/região
  $uf = $getUF($row);
  if ($uf !== '') {
    if (!isset($ufs[$uf]))
      $ufs[$uf] = 0.0;
...
// Top3%
$top3Sum = 0.0;
for ($i = 0; $i < min(3, count($top10)); $i++)
  $top3Sum += (float) $top10[$i]['valor'];
...
$margTop10 = array_slice($margTop50, 0, 10);
...
if (count($top10) > 0) {
  ...
  if ($daysNoBuy >= 10)
    $alerta = 'Alerta: Top 1 sem compra há ' . $daysNoBuy . ' dia(s).';
}

// NOVO: ordenar geografia
$regioesList = [];
foreach ($regioes as $nome => $valor) {
  $regioesList[] = [
    'regiao' => $nome,
    'valor' => round((float) $valor, 2),
    'pct' => $totalValorMes > 0 ? round(((float) $valor / $totalValorMes), 6) : 0.0,
  ];
}
usort($regioesList, fn($a, $b) => ((float) $b['valor'] <=> (float) $a['valor']));

$ufsList = [];
foreach ($ufs as $uf => $valor) {
  $ufsList[] = [
    'uf' => $uf,
    'valor' => round((float) $valor, 2),
    'pct' => $totalValorMes > 0 ? round(((float) $valor / $totalValorMes), 6) : 0.0,
    'regiao' => $ufToRegiao($uf),
  ];
}
usort($ufsList, fn($a, $b) => ((float) $b['valor'] <=> (float) $a['valor']));

$ufsMap = [];
foreach ($ufsList as $row) {
  $ufsMap[$row['uf']] = [
    'valor' => $row['valor'],
    'pct' => $row['pct'],
    'regiao' => $row['regiao'],
...
'margem' => [
  ...
  'geografia' => [
    'regioes' => $regioesList,
    'ufs_ranking' => array_values(array_filter($ufsList, fn($r) => (float) ($r['valor'] ?? 0) > 0)),
    'ufs_map' => $ufsMap,
  ],
