...
========================================================= */
    ...
    #createUserModal.modal.is-open .modal__backdrop {
      opacity: 1;
    }
    ...
    #createUserModal .modal__body::-webkit-scrollbar {
      width: 6px;
    }

    #createUserModal .modal__body::-webkit-scrollbar-track {
...
========================================================= */
    ...
    .admin-users .table-wrap {
      max-height: 560px;
      /* altura aproximada para ~10 usuários */
      overflow-y: auto;
      overflow-x: hidden;
    }
...
<body class="page">
  ...
  <main class="container admin-users">
    <div class="page-title">
      ...
      <div class="ui-actions">
        <button type="button" class="btn-icon" id="btnOpenCreate" aria-haspopup="dialog"
          aria-controls="createUserModal">
    ...
    <?php if ($success): ?>
      <div class="alert alert--ok" style="margin-bottom:12px;"><?= htmlspecialchars($success, ENT_QUOTES, 'UTF-8') ?>
      </div>
    <?php endif; ?>
      ...
      <div class="modal__panel" role="document">
        ...
        <div class="modal__body">
          <form class="form" id="createUserForm" autocomplete="off" enctype="multipart/form-data">
            <input class="offscreen-bait" type="email" name="fake_email" autocomplete="username" aria-hidden="true"
              tabindex="-1">
            <input class="offscreen-bait" type="password" name="fake_pass" autocomplete="current-password"
              aria-hidden="true" tabindex="-1">

            <label class="field" for="name">
              <span class="field__label">Nome Completo</span>
              <input class="field__control" id="name" name="name" type="text" required autocomplete="off"
                spellcheck="false" autocapitalize="off" />
            </label>

            <label class="field" for="adm_email">
              <span class="field__label">E-mail</span>
              <input class="field__control" id="adm_email" name="adm_email" type="email" required autocomplete="email"
                inputmode="email" spellcheck="false" autocapitalize="off" />
            </label>

            <label class="field" for="adm_pass">
              <span class="field__label">Senha</span>
              <input class="field__control" id="adm_pass" name="adm_pass" type="password" required
                autocomplete="new-password" minlength="6" />
            </label>

            <label class="field" for="phone">
              <span class="field__label">Telefone</span>
              <input class="field__control" id="phone" name="phone" type="tel" placeholder="(11) 99999-9999"
        ...
        <table class="table" id="usersTable">
          ...
          <tbody>
            <?php foreach ($users as $row): ?>
              ...
              $pp = trim((string) ($row['profile_photo_path'] ?? ''));

              if ($pp !== '') {
                if ($pp[0] !== '/') {
                  $pp = '/admin/uploads/profile_photos/' . ltrim($pp, '/');
                }
              } ?>
              <tr data-name="<?= htmlspecialchars(mb_strtolower($name), ENT_QUOTES, 'UTF-8') ?>"
                ...
                <td class="col-photo">
                  <span class="cell-center">
                    ...
                    <?php else: ?>
                      <span class="avatar avatar--placeholder" aria-label="Sem foto" title="Sem foto"><span
                ...
                <td class="col-last"><?= htmlspecialchars((string) ($row['last_login_at'] ?? '—'), ENT_QUOTES, 'UTF-8') ?>
                </td>

                <td class="col-actions">
                  <a class="link link--pill" href="/admin/users/user_edit.php?id=<?= (int) $row['id'] ?>">Editar</a>
                </td>
              </tr>
            <?php endforeach; ?>
            ...
            <?php if (!$users): ?>
              <tr>
                <td colspan="8" class="muted">Nenhum usuário cadastrado.</td>
              </tr>
            <?php endif; ?>
          </tbody>
        </table>
  ...
  <script src="/assets/js/users.js?v=<?= filemtime(APP_ROOT . '/assets/js/users.js') ?>"></script>
  <script>
    /* =========================================================
    ...
    function prependUserRow(u) {
      ...
      if (u.profile_photo_path) {
        photo = String(u.profile_photo_path);
        if (!photo.startsWith('/')) {
          photo = '/admin/uploads/profile_photos/' + photo.replace(/^\/+/, '');
        }
        photo = escapeHtml(photo);
      } const photoTd = photo
...
<td class="col-actions"><a class="link link--pill" href="/admin/users/user_edit.php?id=${Number(u.id || 0)}">Editar</a></td>  `;
    ...
    if (createForm) {
      createForm.addEventListener('submit', async (e) => {
        ...
        try {
          ...
          const res = await fetch('/admin/users/users_create.php', {
            method: 'POST',
            body: fd,
